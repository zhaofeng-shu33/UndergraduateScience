...
public:
  ...
  void Solve()
  {
    ...
    { //从矩阵第一行到第order-1行进行 高斯消去
      double max1 = initial_address[k * order + k];
      int row = k;
    ...
    if (fabs(initial_address[order * order - 1]) < 0.001)
    {
      ERROR = 1;
      return;
    }
    ...
    for (int i = order - 2; i >= 0; i--)
    {
      ...
      if (fabs(initial_address[i * order + i]) < 0.001)
      {
        ERROR = 1;
        return;
      }
  ...
  void Update_Solution_Vector_SOR(double w) {
      for (int i = 0; i < order; i++)
      {
        double _val = b[i];
        for (int j = 0; j < order; j++)
        {
          _val -= initial_address[i * order + j] * solution_vector[j];
        }
        solution_vector[i] += w * _val / initial_address[i * order + i];
      }
  }
  double l2_err(double *true_solution_vector) {
    double _val = 0;
      for (int i = 0; i < order; i++)
      {
        _val += pow(true_solution_vector[i] - solution_vector[i], 2);
      }
    return sqrt(_val);
  }
  void Solve_Iterate_SOR(double w, int k, double *initial_solution_vector=nullptr, double * true_solution_vector=nullptr)
  {
    // k is the number of iteration time
    // initial value is set to zero
    // using Jacobi method for iteration
    
    if (initial_solution_vector != nullptr)
    {
      for (int i = 0; i < order; i++)
      {
        solution_vector[i] = initial_solution_vector[i];
      }
    }
    else
    {
    ...
    for (int r = 0; r < k; r++)
    {
      Update_Solution_Vector_SOR(w);
      if(true_solution_vector != nullptr) {
        std::cout << l2_err(true_solution_vector) << std::endl;
      }
    }
